...
int64_t RowTable::PredicatedColumnSum(int32_t threshold1, int32_t threshold2) {
  ...
  for(int32_t row=0; row<num_rows_; row++ ){
    ...
    int32_t col1 =*(int32_t*)(rows_ + FIXED_FIELD_LEN*(row*num_cols_+1));
    int32_t col2 =*(int32_t*)(rows_ + FIXED_FIELD_LEN*(row*num_cols_+2));
...
int64_t RowTable::PredicatedAllColumnsSum(int32_t threshold) {
  ...
  int64_t res = 0;
  for(int32_t row = 0; row < num_rows_; row++ ){
      if( (*(int32_t *)(rows_ + FIXED_FIELD_LEN * row * num_cols_)) > threshold){
